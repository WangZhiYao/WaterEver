        ...
        if (deniedPermissions.size() > 0) {
            ...
            if (rationales.size() > 0 && !mIsPositive) {
                if (mListener != null) {
                    ...
                    mListener.onShowRationale(rationales.toArray(rationaleArray));
                }
                return;
            }
            if (object instanceof Activity) {
                String[] deniedPermission = new String[deniedPermissions.size()];
                ActivityCompat.requestPermissions((Activity) object, deniedPermissions.toArray(deniedPermission), requestCode);
            } else if (object instanceof Fragment) {
                String[] deniedPermission = new String[deniedPermissions.size()];
                ((Fragment) object).requestPermissions(deniedPermissions.toArray(deniedPermission), requestCode);
            } else {
